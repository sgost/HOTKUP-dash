...
<div class="app_container" style="background-color: whitesmoke;grid-template-columns:1fr 3fr; height: 100%">
            ...
            <div class="adk_grid_list_content custom-scroll-bar" style="display:grid;row-gap:7.5px">
                <div  v-bind:key="item.id"
                    ...
                    <div class="list-item-row-1" style="display:flex;column-gap:5px">
                      <div class="task-sno" style="color: black">
                        {{item.name}}
                      </div>
    ...
    <div class="pane-detail-view" style=";overflow-y:hidden">
        ...
        <div v-show="!isUserChosen" style="display:grid;grid-template-rows:1fr;overflow-y:hidden">
            ...
            <div v-show="displayNewForm" style="display:grid;grid-template-rows:1fr;overflow-y:hidden">
                <new-form-maker v-on:refreshList="refreshPaneList()" v-bind:resetTime="resetTime"/>
            </div>
        </div>
        <div v-show="isUserChosen" style="display:grid;grid-template-rows:1fr;overflow-y:hidden">
            <edit-form-maker v-bind:id="selectedUser.id" v-on:refreshList="refreshPaneList()"/>
        </div>
    </div>
</div>
...
export default {
    ...
    methods: {
        ...
        updateFetchURL(){
            ...
            this.url = "./task-form-templates/list/" + this.pageNumber + "/" + searchQuery; // + userID;
