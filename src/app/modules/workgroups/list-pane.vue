...
<div class="app_container" style="background-color: whitesmoke;grid-template-columns:1fr 2fr; height: 100%">
    <div class="desktop pane_list_view" style="padding: 15px 15px 15px 15px; overflow-y: hidden; display: grid; background: rgb(242, 242, 242); color: rgb(51, 51, 51);">
        ...
        <div class="adk_grid_toolbar" style="grid-template-columns: auto 1fr;margin-right: 10px;">
            ...
            <div style="display: grid;grid-gap: 10px;grid-template-columns: auto auto;place-self: center end;text-align: right;">
                ...
                <a uk-tooltip="Add New Workgroup" v-on:click="newItem()" class=" uk-button uk-button-small uk-button-default ui_button_small_green ui_button_small"
                ...
                <div  v-bind:key="item.id"
                    ...
                    <div class="list-item-row-3">
                      <div style="display:flex;column-gap:10px">
                        ...
                        <div class="task-status" style="align-self: flex-end;">{{item.status}}</div>
    ...
    <div class="pane-detail-view" style=";overflow-y:hidden">
        ...
        <div v-show="!isWorkgroupChosen" style="display:grid;grid-template-rows:1fr;overflow-y:hidden">
            ...
            <div v-show="displayNewForm" style="display:grid;grid-template-rows:1fr;overflow-y:hidden">
                <new-workgroup-form v-on:refreshList="refreshPaneList()" v-bind:resetTime="resetTime"/>
            </div>
        </div>
        <div v-show="isWorkgroupChosen" style="display:grid;grid-template-rows:1fr;overflow-y:hidden">
                <edit-workgroup-form v-bind:id="selectedWorkgroup.id" v-on:refreshList="refreshPaneList()"/>
        </div>
    </div>
</div>
...
export default {
    ...
    methods: {
        ...
        showDetails(itemId, idx) {
          ...
          this.selectedWorkgroup = {id:itemId};
          this.isWorkgroupChosen = true;
        },
        ...
        modifyCategoryStatus(idParam, shouldEnable){
            console.log("Workgroup : " + idParam + (shouldEnable)?" will be enabled.":" will be disabled.");
        },
        updateFetchURL(){
        ...
        handleData(dataResponse){
            ...
            if (dataResponse.data["actionResult"] === 1) {
                ...
                let listOfWorkgroups = (dataResponse.data["bean"]);//.slice(0, 10);
                ...
                if(listOfWorkgroups.length <20)
                {
                  this.allRecordsFetched = true;
                }

                listOfWorkgroups.forEach(workgroup => {

                    if(workgroup.createdOn !== null)
                    {
                        workgroup.createdOn = new Date(workgroup.createdOn).toLocaleString()
                                                                 .slice(0, 17)
                                                                 .replace(',', '')
                                                                 .replace('T',' ');
                    }

                    if(workgroup.isOnline!==null && workgroup.isOnline===1)
                    {
                        workgroup.isOnline = "LIVE";
                    }

                    this.list.push(workgroup);
                });
